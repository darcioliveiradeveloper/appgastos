...
export default function Receitas(){
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-emerald-500 to-teal-600 text-white p-6 rounded-2xl shadow flex justify-between items-center">
        <div><h1 className="text-2xl font-extrabold">💚 Receitas</h1><p className="text-emerald-100 text-sm">Entradas do mês</p></div>
      ...
      <form onSubmit={salvar} className="bg-white p-4 rounded-xl shadow grid md:grid-cols-5 gap-2">
        <input placeholder="Categoria (Salário, Freelance)" value={form.categoria} onChange={e=>setForm({...form,categoria:e.target.value})} className="border p-3 rounded-xl" required/>
