    ...
    <div className="space-y-2 w-full max-w-6xl mx-auto overflow-hidden">
      ...
      <Grid cols={3}>
        {cartoes.map(c=>(
          <div key={c._id} className="rounded-2xl p-5 text-white shadow w-full overflow-hidden" style={{background: c.cor}}>
            <p className="font-extrabold">{c.nome}</p><p className="text-sm opacity-80">Limite R$ {c.limite.toFixed(2)} • Venc {c.vencimento}</p>
      ...
      <CardPadrao>
        ...
        <form onSubmit={criarCartao} className="grid md:grid-cols-5 gap-2">
          ...
          <input placeholder="Limite" type="number" value={form.limite} onChange={e=>setForm({...form,limite:e.target.value})} className="border border-slate-200 p-3 rounded-xl" required/>
          <input placeholder="Venc dia" type="number" value={form.vencimento} onChange={e=>setForm({...form,vencimento:e.target.value})} className="border border-slate-200 p-3 rounded-xl"/>
